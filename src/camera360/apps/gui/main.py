    ...
    with ui.tab_panels(tabs, value='Main').classes('w-full'):
        with ui.tab_panel('Main'):
            ...
            with ui.card().classes('w-6/12') as card:
                ...
                ui.run_javascript(f'attachHls({video.id}, "/video/stream/playlist.m3u8");')
...
async def grab_video_frame(rest_of_path) -> Response:
    return Response(content=base64.decodebytes(
        await supervisor.preview(
            filename=rest_of_path)), media_type='text/plain')
