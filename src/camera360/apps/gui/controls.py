...
from camera360.lib.camera.controls import MenuItem, NumericControl, BaseControl, BooleanControl
...
def integer(control: NumericControl, on_change=None) -> None:
    ui.label(control.name)
...
def boolean(control: BooleanControl, on_change=None) -> None:
    ui.label(control.name)
    ui.label(control.description)
...
def create_control(control: BaseControl, on_change: Callable) -> None:
    ...
    with ui.card().classes("w-2/12"):
        callable(control, on_change=on_change)


_type_to_control = {"menu_item": menuoption, "integer": integer, "boolean": boolean}
