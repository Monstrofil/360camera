...
def menuoption(control: MenuItem, on_change=None) -> None:
    ...
    combobox = ui.select(options=control.options,
                         value=control.options[0], on_change=on_change)
...
def integer(control: Integer, on_change=None) -> None:
    ...
    with ui.row():
        ...
        input = ui.number(value=30, min=control.minimum,
                          max=control.maximum, on_change=on_change)
